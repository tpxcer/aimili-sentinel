...
function updateIpTypeFilter() {
  ...
  const selectedValue = select.value;
  const ipTypes = Array.from(new Set(nodes.map(n => n.ip_type).filter(Boolean))).sort((a, b) => {
    return translateIpType(a).localeCompare(translateIpType(b), "zh-Hans-CN");
  });

  const currentOptions = Array.from(select.options).map(o => o.value).filter(Boolean);
  if (JSON.stringify(ipTypes) === JSON.stringify(currentOptions)) {
    return;
  }

  select.innerHTML = '<option value="">所有 IP 类型</option>' +
    ipTypes.map(t => `<option value="${esc(t)}">${esc(translateIpType(t))}</option>`).join("");

  if (ipTypes.includes(selectedValue)) {
    select.value = selectedValue;
  } else {
    select.value = "";
  }
...
function normalizedLatency(node) {
  const latency = Number(node && node.latency_ms);
  return latency > 0 ? latency : Number.POSITIVE_INFINITY;
}

function nodeStatusRank(node) {
  if (!node) return 9;
  if (node.active || node.id === state.active_openvpn_node_id) return 0;
...
function sortNodesForDisplay(list) {
  const sortOrder = $("sort_order").value || "latency_asc";
  return [...list].sort((a, b) => {
    if (!a || !b) return 0;

    const aActive = a.active || a.id === state.active_openvpn_node_id;
    const bActive = b.active || b.id === state.active_openvpn_node_id;
    if (aActive !== bActive) return aActive ? -1 : 1;

    const aLatency = normalizedLatency(a);
    const bLatency = normalizedLatency(b);
    const aLatencyKnown = Number.isFinite(aLatency);
    const bLatencyKnown = Number.isFinite(bLatency);
    const aScore = Number(a.score || 0);
    const bScore = Number(b.score || 0);
    const idCompare = (a.id || "").localeCompare(b.id || "");

    if (sortOrder === "latency_desc") {
      if (aLatencyKnown !== bLatencyKnown) return aLatencyKnown ? -1 : 1;
    ...
    if (sortOrder === "status") {
      const aRank = nodeStatusRank(a);
      const bRank = nodeStatusRank(b);
...
function getFilteredNodes() {
  ...
  const filtered = nodes.filter(n => {
    ...
    if (selectedStatus === "active" && !(n.active || n.id === state.active_openvpn_node_id)) {
      return false;
    }
    if (selectedStatus && selectedStatus !== "active" && n.probe_status !== selectedStatus) {
      return false;
    }
    const searchStr = [
  ...
  return sortNodesForDisplay(filtered);
}
...
$("sort_order").onchange=()=>{ currentPage = 1; render(); };
