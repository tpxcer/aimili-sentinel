...
if [ -f "${INSTALL_DIR}/.local_dev" ]; then
    ...
    if [ -d "${REPO_DIR}/.git" ]; then
        echo -e "  -> 目录 ${REPO_DIR} 已存在，正在更新并强制覆盖本地源码..."
        cd "${REPO_DIR}"
        git fetch --all || true
        ...
        echo -e "  -> 正在克隆 GitHub 仓库 ${GITHUB_URL} 到 ${REPO_DIR} (分支: ${DEPLOY_BRANCH}) ..."
        if git clone -b "${DEPLOY_BRANCH}" "${GITHUB_URL}" "${REPO_DIR}"; then
            echo -e "${GREEN}  -> 克隆成功！${PLAIN}"
        else
            echo -e "  -> 尝试默认克隆..."
            if git clone "${GITHUB_URL}" "${REPO_DIR}"; then
                cd "${REPO_DIR}"
                git checkout "${DEPLOY_BRANCH}" || git checkout -b "${DEPLOY_BRANCH}" "origin/${DEPLOY_BRANCH}" || true
                echo -e "${GREEN}  -> 克隆成功！${PLAIN}"
            else
